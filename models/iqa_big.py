...
def extract_feature(save_output):
    ...
    # x3 = save_output.outputs[3]
    # x5 = save_output.outputs[5]
    # x6 = save_output.outputs[6]
    # x7 = save_output.outputs[7]
    x13 = save_output.outputs[13]
    x27 = save_output.outputs[27]
    # x10, _weights10 = save_output.outputs[10]
    # x11, _weights11 = save_output.outputs[11]
    # feat = x3[:,1:]
    feat = torch.cat((x13[:, 1:], x27[:,1:]), dim=2)
    return feat
...
class IQA(nn.Module):
    def __init__(self, in_channels=3, embed_dim=1024, num_outputs=1, dim_mlp_head=512,
        ...
        self.vit = timm.create_model('vit_huge_patch14_224_in21k', pretrained=True)
        ...
        embed_dim = 1280
        self.conv_first = nn.Conv2d(embed_dim*2, embed_dim, 1, 1, 0)
        self.conv_second = nn.Conv2d(embed_dim*2, embed_dim, 1, 1, 0)
        self.merge = nn.Conv2d(embed_dim*2, embed_dim, 1, 1, 0)

        self.rb1 = RB(embed_dim)
        self.rb2 = RB(embed_dim)
        self.rb3 = RB(embed_dim)
